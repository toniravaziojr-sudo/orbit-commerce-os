...
A tabela de auditoria raw chama-se **`whatsapp_webhook_raw_audit`** (versões anteriores citavam incorretamente `meta_webhook_audit_raw`). A partir de **F2.13.2.B (11/05/2026)** novos registros gravam `body_preview` como **resumo estrutural JSON sanitizado** (cap 2 KB) via `summarizeWebhookBody()` e `headers_json` com **allowlist canônica** via `safeHeaders()` — ambos em `_shared/pii.ts`. `body_sha256`, `signature_header`, `content_length`, `remote_ip`, `user_agent` e `wa_message_ids` permanecem para forense e dedupe. `whatsapp_inbound_messages.raw_payload` e `agenda_command_log` continuam fora deste escopo: serão tratados em F2.13.2.C e fases subsequentes.
