...
> **Migrado de:** `docs/regras/loja-virtual.md`  
...
| Data | Alteração |
|------|-----------|
| 2026-03-09 | **REGRA DE PARIDADE DESKTOP-MOBILE (v8.4.1)**: Regra fundamental documentada — Desktop e Mobile DEVEM ter funcionalidades IDÊNTICAS. A única diferença permitida é responsividade (tamanho, layout, espaçamento). Se um botão funciona no mobile, DEVE funcionar no desktop e vice-versa. Correções: (1) Header desktop `.slice(0,8)` removido — sem limite de itens no menu (paridade com mobile). (2) Cart drawer Edge enriquecido com calculadora de frete, cupom, controles de quantidade +/−, breakdown subtotal/frete/desconto/total — paridade com SPA `MiniCartDrawer.tsx`. (3) Mobile menu `.slice(0,12)` removido. |
| 2026-03-08 | **MENU URL RESOLUTION (v8.2.3)**: Helper `resolveMenuItemUrl()` no edge compiler resolve `item_type`+`ref_id` → URL válida. Corrige links `href="#"` em header/mobile/footer. Filtra páginas não publicadas. Auditoria global: `category-page-layout.ts` (badges, onclick fix), `product-details.ts` (theme bleeding fix), hydration JS validado. Todas as correções são globais do sistema (não específicas de tenant). |
...
| 2026-03-10 | **MINI CART SINGLETON**: MiniCartDrawer agora é instância única no layout (MiniCartContext). Removidas 5 instâncias duplicadas em ProductGridBlock, FeaturedProductsBlock, ProductCarouselBlock, CategoryPageLayout e BlockRenderer que causavam conflitos de estado e sobreposição de drawers ao navegar entre páginas |
| 2026-03-10 | **SUPPORT WIDGET STOREFRONT (v1.1)**: Widget flutuante de atendimento na loja pública. Configurável via Builder (Atendimento → ThemeSettings). Suporta 3 modos: Chat interno, WhatsApp, ou Ambos. Edge Function `storefront-html` injeta HTML do widget via `generateSupportWidgetHtml()` antes de `</body>`. **v1.1 (Edge Chat Drawer)**: Botão de chat agora abre drawer inline com formulário nome/email → inicia conversa → envia mensagens → recebe resposta IA. Tudo em vanilla JS sem dependência de SPA React. O botão mudou de `<a href="#">` para `<button>` para não adicionar `#` na URL. Componente SPA: `SupportChatWidget.tsx` (usado apenas em rotas SPA). Configuração: `themeSettings.supportWidget` (enabled, type, whatsappNumber, whatsappMessage, buttonColor, position). Oculto automaticamente em rotas `/checkout`. **Lead Capture**: Ao preencher nome/email no chat, o lead é registrado automaticamente via `marketing-form-submit` com `source: 'support_chat'`. A lista "Leads site" (slug: `leads-site`) é auto-criada se não existir, com tag vinculada. |
| 2026-03-10 | **CLOUDFLARE WORKER CACHE (v1.0)**: Camada de cache HTML adicionada ao Worker `shops-router`. O Worker agora cacheia respostas HTML da Edge Function `storefront-html` usando a Cache API do Cloudflare nos PoPs brasileiros (SP/RJ). TTL: 15 minutos. Stale-while-revalidate: 24h. Header `X-CC-Cache: HIT/MISS/BYPASS` para diagnóstico. Preview mode (`?preview`, `?draft`) bypassa o cache. Invalidação: já integrada via `storefront-cache-purge` (Cloudflare API purga tanto CDN quanto Cache API). Arquivo: `docs/cloudflare-worker-template.js`. |
| 2026-03-10 | **NEWSLETTER POPUP EXIT INTENT FIX v2**: Refinado trigger `exit_intent`. (1) **Desktop**: Apenas `mouseleave` com `clientY <= 0` — dispara somente quando mouse sai pela borda superior do viewport (em direção ao botão fechar/abas). Removido `mousemove` agressivo que disparava ao navegar perto do topo. (2) **Mobile**: Detecção por scroll-up repetido no topo da página — quando usuário está em `scrollY <= 20` e tenta rolar para cima 2+ vezes (gesto de "querer sair"). (3) Delay de 2.5s antes de ativar. (4) Flag `eiFired` impede re-trigger. (5) Uma vez por sessão. Aplicado em: Edge HTML (`storefront-html`) e SPA React (`NewsletterPopupBlock.tsx`). |
| 2026-03-10 | **PRERENDER CACHE STALE BUG**: Identificado que migrações de invalidação (`UPDATE SET status='stale'`) desativavam todas as páginas pré-renderizadas sem re-trigger do job `storefront-prerender`. Resultado: loja SEMPRE em live-render (~4s TTFB) em vez do fast-path (~100ms). **REGRA CRÍTICA**: Nunca marcar páginas como `stale` via migração sem chamar `storefront-prerender` em seguida, OU usar a abordagem correta: chamar `storefront-prerender` que faz o ciclo completo (gera pending → atomic_activate). Para invalidações pontuais (ex: correção de popup), preferir `storefront-cache-purge` (CDN only) em vez de alterar status do prerender. |
| 2026-03-10 | **THEME INJECTOR FIX**: StorefrontThemeInjector agora atualiza CSS in-place sem remover no unmount. Correção do bug "tela branca" e flicker de branding ao navegar de/para checkout. PageColorsInjector mantém cleanup correto apenas para overrides de página |
