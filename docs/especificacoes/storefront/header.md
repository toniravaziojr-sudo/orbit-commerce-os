...
> **Migrado de:** `docs/regras/header.md`  
...
| 2026-03-10 | **PARIDADE MENUS v8.5.0**: Corrigidas 6 discrepâncias no Edge compiler `header.ts`: (1) ChevronDown agora aparece em TODOS os estilos (não só classic); (2) "Ver todos" aparece em classic E elegant (não só classic); (3) Adicionado suporte a sub-submenus (3º nível) com `.sf-sub-dropdown` hover; (4) Dropdown posicionado no centro (`left:50%;transform:translateX(-50%)`) como no SPA; (5) Font-size do trigger corrigido de 14px para 12px (paridade com `text-xs`); (6) Cores de texto/fundo agora usam CSS vars (`--theme-popover`, `--theme-popover-foreground`, `--theme-muted-foreground`) em vez de `#1a1a1a` hardcoded. Separadores usam `rgba` com opacidade variável por estilo. Chevron rotaciona 180° no hover via CSS (`.sf-dropdown:hover .sf-dropdown-trigger svg:last-child`). |
| 2026-03-10 | **FIX MENU ITEMS MISSING**: Limite de categorias na query base do `storefront-html/index.ts` aumentado de `limit(10)` para `limit(200)`. Itens de menu do tipo `category` que referenciavam categorias além das 10 primeiras eram silenciosamente descartados por `resolveMenuItemUrl()` (retornava `null` se a categoria não existisse na lista limitada). Isso causava menus incompletos em lojas com mais de 10 categorias. |
| 2026-03-12 | **MENU AUTO-UPDATE (v2.0.0)**: Salvar menu no admin agora atualiza a loja pública automaticamente, sem republicar tema. Implementado `menuAutoUpdate()` em `storefrontCachePurge.ts` com debounce de 5s: marca páginas `stale` → purga CDN → dispara `storefront-prerender` com `trigger_type: 'menu_update'`. Usa `published_content` (nunca `draft_content`). Fallback seguro: se prerender falhar, live-render (~5s) garante acesso. Toast de republicação manual removido. |
