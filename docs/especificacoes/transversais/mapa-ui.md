...
| 17 | Marketing Avançado | Gestor de Tráfego IA | `/ads` | marketing_avancado | — | — | Sub-tab **Plano Estratégico** opera em modo fail-closed: plano incompleto mostra badge **Plano incompleto**, banner de pendências no modal e botão **Aprovar plano** desabilitado; plano válido após ajuste não pode manter tarja residual de incompleto e o feedback do ajuste deve aparecer em **Aprendizado da IA** como sugerido. **Supremacia do Prompt Estratégico (v6.20 — finalizada em 2026-06-17):** o prompt estratégico do lojista (global e por conta) é a fonte de verdade do Gestor. Nas telas de **Configurações Gerais** e **Configuração por Conta de Anúncios**, logo acima do campo de prompt, há um bloco fixo informando que o prompt tem prioridade máxima e que conflitos viram **avisos** (nunca bloqueios), renderizados no mesmo local pelo painel "Avisos do prompt estratégico". **Descontinuado e removido em 2026-06-17:** o bloco "Regras da Marca para Criativos" (promessa principal aprovada, claims permitidas/proibidas, restrições, tom de voz, do_not_do) e a tela "Configuração Global de IA" não existem mais no Gestor de Tráfego — refino editorial passa a ser feito 100% via prompt estratégico + feedback nas propostas. Os campos `tenant_brand_context` seguem vivos apenas para o **modo Vendas (WhatsApp)**. **Análise estratégica v1.3.3:** botão manual inicia rápido, popup/card mostram "Rodando análise estratégica" / "Analisando agora" e a rodada é finalizada em background pelo motor estratégico; falhas por tempo/provedor aparecem como erro claro, sem deixar a UI presa. **Dialogs de decisão (2026-06-17):** ao abrir Ajustar, Recusar ou confirmação de aprovação a partir do modal completo, a proposta permanece aberta no fundo; cancelar/fechar o secundário retorna ao modal primário. **H.4.1 — Prontidão e Geração de Criativos:** abaixo de cada proposta aprovada aparece um card de prontidão; bloqueia apenas por falhas técnicas reais (conexão Meta, página, pixel, orçamento, URL/UTM, imagem do produto, logo/paleta, tabela de preços de IA). Itens editoriais viram avisos. Botão **"Gerar criativos"** com custo em créditos abre diálogo de confirmação obrigatório ("Isso iniciará processamento de IA" / "Nada será enviado ao Meta agora"); fechar não consome créditos. Memória: `mem://constraints/strategic-prompt-supremacy`. |
...
| 30 | Marketplaces | Mercado Livre | `/marketplaces/mercadolivre` | marketplaces | `mercadolivre` | — | Badge "Upgrade" se bloqueado. **Abas: Conexão, Anúncios, Métricas** (aba "Pedidos" removida em v3.3 — 2026-06-26 — pedidos do ML entram no módulo central `/orders` com filtro de Origem `mercadolivre`, badge ML via `OrderSourceBadge`). Aba Anúncios usa assistente em lote com etapas Produtos, Categorias, Títulos, Descrições, Características, Condição, Tipo, Preços e Frete; Preços ajusta apenas o valor específico do anúncio no ML, sem alterar o cadastro do produto. |
...
| 13 | Diretrizes Comerciais | `/platform/commercial-guidelines` | **Onda H.4.1 Fase 2 (2026-06-16):** base global de diretrizes Meta/Google/TikTok por categoria inferida (cosmético, suplemento, moda, eletrônico, pet, alimento, infantil). Aba por plataforma, badge Ativa/Revisão pendente, versão, data da última verificação. Botões: Carregar baseline, Verificar agora, Editar (aprovação humana). Cron mensal automático (`platform-guidelines-monthly-refresh`, dia 1 03:00 UTC) usa Firecrawl + IA para detectar mudanças nas políticas oficiais. Mudança detectada NUNCA bloqueia geração — serve versão anterior até admin aprovar. Restrito a `platform_admin`. |
...
- `/products` — Produtos. Na criação/edição, o formulário usa o scroll global do `AppShell`; a barra fixa inferior do formulário não pode criar área branca extra no fim do documento. **Aba "Visão IA"** (somente em edição) exibe a inteligência comercial editável da IA — papel comercial, tipo, produto-base relacionado, complementares, "quando recomendar/não recomendar". Persistida em `ai_product_commercial_payload` + `ai_product_relations`. Não afeta runtime da IA nesta onda. **Cartão "Classificação Universal" (rev 2026-06-21):** novo cartão na aba principal do cadastro com 4 campos universais usados pela IA de envio aos marketplaces — **Categoria universal** (árvore hierárquica do catálogo do sistema), **Regime regulatório** (Nenhum, ANVISA Cosmético/Saúde/Suplemento/Medicamento, INMETRO, ANATEL, MAPA, DENATRAN, Exército, IBAMA, ISS, Outros — auto-sugerido pela categoria selecionada), **Conteúdo líquido** (valor + unidade ml/l/g/kg/un/m/cm/m²/m³), **Público/Gênero** (Masculino/Feminino/Unissex/Infantil/Não se aplica). Dados consumidos no dialog de envio ao Mercado Livre/Shopee/TikTok Shop. **Cartão "Atributos Cosméticos" (rev 2026-06-21, Etapa 5.5):** aparece somente quando o regime regulatório é ANVISA Cosmético. 9 campos: Dermatologicamente testado, Hipoalergênico, Livre de crueldade, Vegano, Com fragrância (tri-state Sim/Não/Não se aplica), Nome da fragrância, Tipos de cabelo recomendados (multi-tag), Tipos de tratamento (multi-tag), Efeitos esperados. Esses campos viram preenchimento automático no envio ao Mercado Livre. **Botão "Aplicar atributos em lote"** na barra de ferramentas da lista de produtos abre dialog para propagar atributos cosméticos para N produtos selecionados em 1 clique. **Cartão "Marca e Fornecedor" (rev 2026-06-23, v2.5.0):** 5 campos opcionais — **Marca**, **Modelo**, **Linha** (NOVO — ex.: "Calvície Zero", usada no atributo LINE dos marketplaces), **Fornecedor** e **Tipo de Produto** (ex.: "Shampoo", "Balm", "Loção"). Linha tem prioridade absoluta sobre derivações no envio ao ML.
...
- `/ads` — **Onda 3 (rev 2026-06-17) — Reestruturação de abas e fim do conceito "global":** abas principais passam a ser, nesta ordem fixa: **Gerenciador → Chat IA → Aprendizado da IA → Desempenho → Avisos**. Saíram do produto: aba "Insights", aba "Configurações Gerais", sub-aba "Chat IA" dentro de cada conta, botão "Analisar todas as contas (global)", botão "Gerar Insights Agora" e rotina semanal de geração de insights. Nasceu a aba **"Avisos"** — lista cronológica de sinais diagnósticos detectados pelo ciclo da IA por conta (severidade informativo / atenção / urgente, tendência alta/baixa/estável, ações marcar como visto, dispensar, virou proposta). Regra de saída do ciclo da IA por conta: ação concreta → proposta em "Aguardando Ação"; diagnóstico relevante sem ação → aviso na aba Avisos; irrelevante → memória interna. Anti-regressão: aviso ≠ proposta, e nada de "IA global". Detalhe em `docs/especificacoes/marketing/gestor-trafego.md` (seção "Onda 3"). Memória: `mem://constraints/ads-no-global-and-avisos-not-proposals`.
...
- `/marketplaces/mercadolivre` — Mercado Livre (FeatureGated). **Wizard de anúncio (rev 2026-06-21):** após escolher a categoria do ML, aparece o painel "Atributos para o anúncio" com 3 blocos (Preenchido / Revisar / Faltando). O motor de IA `meli-resolve-attributes` cruza cadastro + derivações + dicionário universal + Gemini e devolve os valores; publicação bloqueada enquanto houver atributos obrigatórios faltando. **Ajuste de preço no wizard (v2.5.0 — 2026-06-23):** abaixo do campo Preço da etapa de revisão, 3 controles permitem ajuste rápido — **Aplicar desconto %** (reduz X% do preço atual), **Aplicar acréscimo %** (aumenta X% para cobrir taxa do ML) e **Restaurar do cadastro** (volta ao `products.price`). O ajuste vale apenas para o anúncio ML — o preço interno do produto não é alterado. **Política manual-only (v2.5.0):** não existe envio automático ao ML. Todo PUT/POST é manual; o cron diário é só leitura. **Aba "Anúncios" reorganizada (v2.4.1 — 2026-06-21):** lista dividida em abas — **Rascunhos** (padrão, anúncios em draft/ready/approved), **Publicados** (apenas `published` e `paused`), **Pendências** (anúncios com `error` **e** `publishing`, com badge destrutivo) e **Inativos** (v2.6 — anúncios em `inactive` finalizados no ML, com `inactive_reason` visível e link "Ver no Mercado Livre (histórico)"). Barra de ações em massa aparece apenas com seleção e contém **dois botões**: **Editar em Lote** (reabre o assistente; quando todos os selecionados já estão publicados, o botão final vira "Atualizar anúncios no Mercado Livre" e sincroniza via API; caso contrário, oferece "Salvar como rascunho" ou "Salvar e publicar no Mercado Livre") e **Excluir Selecionados** (**desativa** os publicados no ML — status `closed`, sai do ar, link público para — e remove daqui; rascunhos são removidos apenas localmente; anúncios nunca aprovados pelo ML pulam o encerramento; aviso v2.6 deixa claro que o ML não permite exclusão definitiva de publicados — eles ficam apenas no histórico interno do ML; botão exibe spinner "Excluindo…"). Botão individual de excluir aparece em **todos os status**. Publicação foi movida para a última etapa do dialog de criação. Removidos da tela principal: "Enviar Todos", "Gerar Títulos", "Gerar Descrições", "Auto-Categorizar" e "Enviar Selecionados" (continuam disponíveis dentro do dialog). **Aba Pendências (v2.5 — 2026-06-22):** linhas cuja causa raiz é dado faltando no cadastro do produto exibem botão **"Abrir cadastro"** que abre `/products?edit=<id>` em **nova aba**. Ao voltar para a aba do navegador, a lista revalida sozinha (focus + visibilitychange, custo zero). **Assistente em lote (v2.5 — 2026-06-22):** passa de 7 para **8 etapas** — entre "Descrições" e "Condição" entra a nova etapa **"Características"**, que renderiza um `MeliAttributesPanel` por anúncio, mostra contador de faltantes, oferece atalho "Abrir cadastro" e bloqueia o "Continuar" enquanto houver atributo obrigatório faltando; ao avançar, persiste o array `attributes` em `meli_listings` por anúncio para que a publicação envie todos os atributos relevantes (obrigatórios + recomendados). **Sincronização bidirecional em tempo real (v2.6 — 2026-06-22):** cabeçalho da tela exibe selo verde **"Sincronizado em tempo real com o Mercado Livre"** (bolinha verde animada) quando o webhook recebeu sinal nas últimas 1h; vira selo âmbar **"Sem sinal recente do Mercado Livre"** caso contrário (cron 05h BRT cobre como fallback). Cada linha mostra ao lado do badge de status um indicador da **origem da última alteração** ("•" azul = feita aqui no sistema; "ML" âmbar = feita no painel do Mercado Livre pelo lojista ou pelo ML), com tooltip de data/hora relativa em BRT. A tabela atualiza em segundos via Supabase Realtime (subscription escopada por tenant, pausada quando a aba do navegador fica oculta para não vazar canais).
...
**"Ajustar proposta" nunca rejeita o plano (rev 2026-06-17).** Para Plano Estratégico e ações sem hierarquia, o botão fecha o modal e abre o diálogo de sugestão por texto livre; ao confirmar, dispara o caminho canônico de ajuste no servidor, que: (1) marca a proposta original como **substituída** (lifecycle "ajuste solicitado"), nunca como rejeitada; (2) registra o texto do lojista como **feedback formal "precisa de revisão"** e cria **aprendizado sugerido** em "Aprendizado da IA"; (3) devolve a **nova versão** em "Aguardando Ação" vinculada à anterior. Falhas (ex.: saldo de IA insuficiente) ficam visíveis com mensagem clara. Anti dupla chamada: nova solicitação para a mesma proposta dentro de 10 min retorna idempotente. Detalhe técnico em `docs/especificacoes/marketing/gestor-trafego.md` §15.

**Editor estruturado.** Continua sendo o drawer da Frente 4.3, agora aberto pelo "Ajustar proposta" de dentro do modal **apenas para Etapa 1 do two_step_v1** (estratégia com estrutura). Seções renomeadas para refletir a hierarquia: **Campanha** · **Conjunto de anúncios** (engloba Público + Segmentação + Exclusões) · **Anúncio** (engloba Produto + Copy + Criativo) · **Feedback para a IA**. Mutations, rascunho, versionamento e feedback permanecem idênticos.
...
- `/ads` — modal estruturado de proposta:
  ...
  - **Aba Anúncio** separa visualmente dois blocos: **Anúncio** (nome, conjunto vinculado, status) e **Criativo do anúncio** (CTA, link, tracking, copy, formato). A árvore lateral mantém um único item "Anúncio N". **v6.21 (2026-06-18) — Card único "Criativo do anúncio":** toda a edição de mídia e copy de um anúncio vive dentro deste único card. Cabeçalho do card (direita): **Editar manualmente** + botão dinâmico **Gerar copys / Regenerar copys** (regenerar abre popup com feedback obrigatório que vira aprendizado). Coluna esquerda (mídia): miniatura + **um único botão "Gerar criativo" que muda para "Regenerar criativo"** no mesmo lugar após a primeira geração; abaixo, em linha compacta, **Enviar do PC · Escolher no Drive · Remover**. Coluna direita (copy): Título / Texto principal / Descrição com mini-botão **Regenerar** ao lado do rótulo (aparece só com conteúdo). Proibido: duplicar botão de gerar/regenerar criativo, expor controles fora do card, mostrar barra externa de IA. Falhas legadas de "produto não encontrado" são neutralizadas por fallback de resolução por produto da campanha, nome/conjunto e slug do link de destino; a miniatura/base visual vem da imagem principal do catálogo.
...
- **No card de cada conta Meta** passa a aparecer apenas um **status inline somente leitura**:
  - **OK:** "Meta conectada. Ativos técnicos sincronizados pela integração." (quando conta de anúncio, Página vinculada aos anúncios, Pixel e API de Conversões estão disponíveis nos ativos reais da integração Meta).
...
- **Backend preservado:** a tabela `ads_meta_production_config` e o hook `useAdsMetaProductionConfig` continuam existindo como **override técnico avançado**, mas o Strategist e os gates devem usar os ativos reais da integração Meta como fonte primária. O componente `MetaProductionConfigCard` deixa de ser renderizado na UI principal; eventual fallback manual fica restrito à área técnica de integração (não é o fluxo principal).
- **Gates por etapa:**
...
- **Refino editorial 100% via prompt estratégico.** O bloco "Promessa e claims aprovados" foi removido do Gestor de Anúncios. Não há mais editor manual de promessa, claims permitidas/proibidas, restrições ou tom de voz para fins de criativos. A IA segue o prompt estratégico + diretrizes das plataformas e ajusta por feedback nas propostas.
