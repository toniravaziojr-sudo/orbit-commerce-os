...
**Sistema:**
- `/integrations` — Hub de integrações (cards de status; card WhatsApp inclui `WhatsAppActivationGuide` full + `WhatsAppDiagnosticCard` + `WhatsAppPinManager` + `WhatsAppOnboardingPinDialog`). **Status do WhatsApp em camadas (v2026-04-20):** todo card/badge exibe vínculo técnico + operação real lado a lado — rótulos: "Vínculo com defeito", "Vínculo incompleto", "Em observação pós-migração", "Aguardando comprovação operacional", "Recepção instável", "Recepção comprometida", "Recebendo normalmente". Nunca exibir só "conectado/desconectado".
- `/index` (Dashboard) — Inclui `WhatsAppActivationGuide` (compact) abaixo do `OrderLimitWarning`. **Card "Saúde do WhatsApp"** mostra blocos "Vínculo técnico" e "Operação real" + banner amarelo "Vínculo trocado, em observação" durante janela de 24h após troca de WABA / Phone Number ID.
