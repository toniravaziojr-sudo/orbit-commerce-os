...
- `/fiscal/configuracoes` — Configurações Fiscais (página dedicada — abas: Configurações Fiscais, Natureza Jurídica, **Integração Focus NFe** *(Lote 1.E — 2026-05-14)*, Outros). **Casa oficial:** `Sistema → Configurações → aba Fiscal` (conteúdo embutido na aba — sem redirecionamento desde rev 2026-04-17c). **Atalho:** botão "Configurações" em `/fiscal` abre `/fiscal/configuracoes?from=fiscal`. **Botão "Voltar" contextual via `?from=`:** `?from=fiscal` retorna para `/fiscal?tab=pedidos`; `?from=settings` (ou sem param) retorna para `/system/settings?tab=fiscal`. **Aba Integração Focus NFe (`?aba=integracao`)** — owner/admin: 4 cards de saúde (Empresa Focus, Certificado A1, Webhook, Ambiente) + indicadores `ready_for_production` / `ready_for_homologation_smoke` + botões **Validar integração fiscal** e **Cadastrar webhook automaticamente**. Fallback manual seguro (URL com token por loja, mascarado, com Revelar/Copiar/Rotacionar) aparece somente quando o cadastro automático falha. UI nunca expõe `FOCUS_NFE_WEBHOOK_SECRET`, PFX, senha do certificado nem `provider_token`. **UI rev 2026-05b:** aba Configurações Fiscais reorganizada em 5 blocos verticais — (1) Cartão de Prontidão Fiscal com checklist acionável de **7 itens** (Dados, Endereço, Parâmetros, Certificado, CNPJ-match, **E-mail do emitente**, Ambiente — o item de e-mail é recomendação amarela e não bloqueia emissão), (2) Identidade da Empresa (Dados+Endereço unificados + nova seção **"Contato do emitente"** com **E-mail** e **Telefone do emitente**, ambos enviados à Focus NFe e usados como remetente do DANFE / impressão no DANFE), (3) Certificado Digital A1 em destaque com botão "Atualizar CNPJ do emitente para X" quando há divergência, (4) Parâmetros Fiscais, (5) Ambiente. Barra de Salvar fixa no rodapé aparece só com alterações pendentes. Não aparece como item próprio na sidebar.
