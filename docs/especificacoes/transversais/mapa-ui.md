...
| 17 | Marketing Avançado | Gestor de Tráfego IA | `/ads` | marketing_avancado | — | — | Sub-tab **Plano Estratégico** opera em modo fail-closed: plano legado/incompleto mostra badge **Plano incompleto**, banner de pendências no modal e botão **Aprovar plano** desabilitado quando `validation_status!='valid'` ou `is_approvable!=true`. Ações frias/prospecção exibem exclusão de clientes ou pendência técnica no nível do adset. A fonte exibida é sempre o payload salvo canônico; retorno intermediário achatado não deve mais entrar na fila como plano novo. **H.4.0 — Promessas, claims e restrições da marca (rev 2026-06-16):** bloco oficial vive em duas camadas dentro deste módulo. (a) **Configuração Global** → expansor "Configuração Global" exibe o bloco com 5 campos (Promessa principal aprovada, Claims permitidas, Observações comerciais/compliance, Claims proibidas, Não fazer) + checkbox "Confirmo que não há restrições adicionais"; persiste em `tenant_brand_context`. (b) **Configuração por Conta de Anúncios** → mesmo bloco como **override opcional** dentro do card de cada conta; persiste em `ads_autopilot_account_configs.chat_overrides.brand_overrides`; vazio = herda do global. Não existe outra tela para esses campos. **H.4.1 — Prontidão e Geração de Criativos (rev 2026-06-16):** abaixo de cada proposta aprovada aguardando criativos aparece um card de prontidão. Quando há pendências (promessa principal, claims, restrições, imagem do produto, categoria, faixa de preço, configuração Meta etc.), o card mostra um aviso amarelo listando o que falta em linguagem simples (PT-BR), com link direto para a tela de origem do dado. Quando tudo está pronto, o card fica azul e exibe o botão **"Gerar criativos"** com o custo estimado em créditos. O botão abre um diálogo de confirmação obrigatório que reforça: "Isso iniciará processamento de IA" e "Nada será enviado ao Meta agora". Só após o clique de confirmação humano os criativos são enfileirados (idempotente, sem dobrar custo em duplo clique) e a proposta avança para o estado de geração. Recusar/fechar o diálogo não consome créditos. **Supremacia do Prompt Estratégico (rev 2026-06-16, v6.20):** nas telas de **Configurações Gerais** e **Configuração por Conta de Anúncios**, logo **acima do campo "Prompt Estratégico / Prompt de Direcionamento Global"**, passa a existir um bloco fixo informando que o prompt do lojista tem prioridade máxima sobre regras manuais, função declarada do produto e diretrizes das plataformas; e que conflitos viram **avisos** nesse mesmo local — nunca bloqueios. Esse bloco também é o ponto oficial de renderização do painel "Avisos do prompt estratégico" da Fase 2. Itens editoriais (descrição, tipo/função do produto, promessa, claims, restrições) deixam de bloquear a geração de criativos e viram avisos. Memória: `mem://constraints/strategic-prompt-supremacy`. |
...
### H.4.0 — Prontidão para geração de criativos (sem UI de gerar ainda)

- **Editor de marca/IA** (`BrandContextEditor`): adicionado bloco "Promessa e claims aprovados" com 3 campos novos (promessa principal aprovada, claims permitidos, observações de compliance) e checkbox "Confirmo que não há restrições adicionais para esta marca". Não criou rota nova nem aba nova; é apenas extensão visual do card de identidade/restrições.
- **Editor de produto** (`ProductForm`, aba "Avançado"): adicionado card "Categoria e restrições para geração de criativos" com select de categoria regulatória (cosmético/capilar, suplemento, outro), textarea de restrições comerciais/legais e checkbox de confirmação de ausência de restrições adicionais. Aparece acima do card "Informação Regulatória" fiscal.
- **Seção de propostas aprovadas em andamento**: sem mudança visual nesta onda. O botão "Aguardando próxima etapa" permanece. A H.4.1 trará botão "Gerar criativos" condicionado a `creative_readiness.status=ready` e o card de bloqueio com até 3 pendências visíveis + "Ver todos".
- Backend: motor puro em `supabase/functions/_shared/ads-autopilot/creativeReadinessGate.ts` (contract `h4_readiness_v1`). 28 testes cobrindo bloqueadores de Meta, proposta, produto, marca e custo. Detalhe em `.lovable/memory/constraints/ads-h4-creative-production-first.md`.
