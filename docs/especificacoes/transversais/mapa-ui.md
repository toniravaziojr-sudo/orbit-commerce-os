...
**ERP:**
- `/fiscal` — Fiscal (abas: **Pedidos de Venda**, Notas Fiscais). "Pedidos de Venda" é o nome interno fiscal para os rascunhos de venda usados na revisão e emissão de NF-e — **não confundir com o módulo `/orders` da loja**. Lista inclui ação em massa **"Enviar à transportadora"** (NFes autorizadas com transportadora gateway — dispara `gateway-attach-fiscal-doc`). **DC-e (Declaração de Conteúdo) removida do fluxo operacional comum (rev 2026-05-14b):** o botão de ação em massa "Emitir DC-e" foi retirado porque o backend ainda é placeholder e depende de integração logística não finalizada — pendência separada de Fiscal/Logística. **Emissão de NF-e exige confirmação obrigatória em todos os pontos (rev 2026-05-14b):** tanto o item "Emitir NF-e" do menu da linha quanto o botão "Emitir NF-e" do rodapé do editor abrem modal de confirmação antes de transmitir; em homologação o rótulo vira "Emitir NF-e de teste" com aviso de "sem valor fiscal real"; em produção o aviso é "valor fiscal real, ação não pode ser desfeita". **Duplicar Pedido de Venda / Duplicar NF (rev 2026-05-14):** o menu de ações em ambas as abas inclui o item de duplicação (rótulo **"Duplicar Pedido de Venda"** na aba Pedidos de Venda, **"Duplicar NF"** na aba Notas Fiscais — disponível para rascunhos, autorizadas e canceladas). A ação abre um diálogo pré-preenchido (`ManualInvoiceDialog` em `mode="duplicate"`) onde o usuário revisa/edita e clica em **"Salvar duplicação"**. O salvar gera sempre um rascunho independente (sem `order_id`, sem chave/XML/protocolo, número fiscal novo) — não emite, não chama provedor fiscal, não toca estoque/financeiro/remessa/e-mail/automação e **não cria/altera registros no módulo `/orders` da loja**. Ao duplicar uma NF (autorizada/cancelada/rejeitada), o novo rascunho aparece na aba **Pedidos de Venda**, não em Notas Fiscais — toast diz "NF duplicada como rascunho em Pedidos de Venda" e o editor abre automaticamente para evitar sensação de "salvou e sumiu". Detalhe em `docs/especificacoes/erp/erp-fiscal.md`.
