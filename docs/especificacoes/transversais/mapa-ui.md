...
- `/marketplaces/mercadolivre` — Mercado Livre (FeatureGated). **Wizard de anúncio (rev 2026-06-21):** após escolher a categoria do ML, aparece o painel "Atributos para o anúncio" com 3 blocos (Preenchido / Revisar / Faltando). O motor de IA `meli-resolve-attributes` cruza cadastro + derivações + dicionário universal + Gemini e devolve os valores; publicação bloqueada enquanto houver atributos obrigatórios faltando. **Aba "Anúncios" reorganizada (v2.4.1 — 2026-06-21):** lista dividida em **3 abas** — **Rascunhos** (padrão, anúncios em draft/ready/approved), **Publicados** (apenas `published` e `paused`) e **Pendências** (anúncios com `error` **e** `publishing` — em envio / em revisão pelo ML, com badge destrutivo). Barra de ações em massa aparece apenas com seleção e contém **dois botões**: **Editar em Lote** (reabre o assistente; quando todos os selecionados já estão publicados, o botão final vira "Atualizar anúncios no Mercado Livre" e sincroniza via API; caso contrário, oferece "Salvar como rascunho" ou "Salvar e publicar no Mercado Livre") e **Excluir Selecionados** (encerra anúncios publicados no ML de forma definitiva — status `closed`, link sai do ar — antes de remover do sistema; rascunhos são removidos apenas localmente; anúncios nunca aprovados pelo ML pulam o encerramento e saem apenas do sistema; botão exibe spinner "Excluindo…" durante a operação). Botão individual de excluir aparece em **todos os status**. Publicação foi movida para a última etapa do dialog de criação. Removidos da tela principal: "Enviar Todos", "Gerar Títulos", "Gerar Descrições", "Auto-Categorizar" e "Enviar Selecionados" (continuam disponíveis dentro do dialog). **Aba Pendências (v2.5 — 2026-06-22):** linhas cuja causa raiz é dado faltando no cadastro do produto exibem botão **"Abrir cadastro"** que abre `/products?edit=<id>` em **nova aba**. Ao voltar para a aba do navegador, a lista revalida sozinha (focus + visibilitychange, custo zero). **Assistente em lote (v2.5 — 2026-06-22):** passa de 7 para **8 etapas** — entre "Descrições" e "Condição" entra a nova etapa **"Características"**, que renderiza um `MeliAttributesPanel` por anúncio, mostra contador de faltantes, oferece atalho "Abrir cadastro" e bloqueia o "Continuar" enquanto houver atributo obrigatório faltando; ao avançar, persiste o array `attributes` em `meli_listings` por anúncio para que a publicação envie todos os atributos relevantes (obrigatórios + recomendados).
