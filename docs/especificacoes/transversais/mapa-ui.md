...
- `/fiscal/configuracoes` — Configurações Fiscais (página dedicada — abas: **Configurações Fiscais**, Natureza Jurídica, Outros). **Casa oficial:** `Sistema → Configurações → aba Fiscal` (conteúdo embutido — sem redirecionamento desde rev 2026-04-17c). **Atalho:** botão "Configurações" em `/fiscal` abre `/fiscal/configuracoes?from=fiscal`. **Voltar contextual via `?from=`:** `?from=fiscal` → `/fiscal?tab=pedidos`; `?from=settings` (ou sem param) → `/system/settings?tab=fiscal`. **UI rev 2026-05d (Validação Fiscal — 2026-05-14):** o card compacto "Validação Fiscal" continua ao lado do bloco **Ambiente de Emissão**, mas mudou de comportamento. O **selo geral** agora pode ser **Pronto / Pronto para teste / Configuração pendente / Erro / Bloqueado** (em vez de Pronto/Atenção/Bloqueado). "Bloqueado" só é usado em produção. Em homologação, quando todos os pré-requisitos do ambiente de teste estão completos, o selo é **Pronto para teste**. Itens internos exibem rótulo específico (ex.: "Cadastrada / Aguardando credencial", "Configure o token de homologação", "Aguardando primeiro retorno") em vez de "Atenção" genérico. O **botão "Ativar recebimento automático de retornos" deixou de ser etapa obrigatória**: o backend tenta ativar automaticamente o recebimento sempre que os pré-requisitos do ambiente atual estão completos (ao validar, ao salvar credenciais ou quando o card detecta o cenário pronto). Em vez do botão obrigatório, agora aparece **"Tentar novamente"** apenas quando há erro real ou ativação pendente. Um item próprio do card mostra o status do **token de homologação/produção da empresa** ("Configurado" / "Configure o token"). Fallback manual seguro (URL com chave única por loja, mascarada, com Revelar/Copiar/Rotacionar) aparece apenas quando a ativação automática falha. UI nunca expõe segredos globais, PFX, senha do certificado nem token do provedor fiscal. URL legada `?aba=integracao` deixou de existir e cai no comportamento padrão (`emitente`). **Demais blocos (rev 2026-05b):** (1) Cartão de Prontidão Fiscal com checklist de 7 itens, (2) Identidade da Empresa (Dados+Endereço + Contato do emitente: e-mail e telefone), (3) Certificado Digital A1 com botão "Atualizar CNPJ do emitente para X" quando há divergência, (4) Parâmetros Fiscais, (5) Ambiente de Emissão + Validação Fiscal lado a lado. **Credenciais do provedor fiscal (rev 2026-05-14):** seção mínima dentro da aba Configurações Fiscais, com dois campos do tipo senha — **Token Focus de Homologação** e **Token Focus de Produção** — exclusivos do tenant (token da empresa do CNPJ do emitente, não da plataforma). Cada campo mostra apenas selo de status (**Configurado** / **Não configurado**); o valor do token nunca é exibido novamente após salvar. Gravação via RPC segura. **Produção fica bloqueada** se o token de produção do tenant não estiver configurado. **Visibilidade:** apenas owner/admin do tenant. **Operator não acessa** a seção de credenciais. Token principal da conta Focus (`FOCUS_NFE_TOKEN`) continua exclusivo de **Plataforma → Integrações → Fiscal** e nunca aparece nesta tela. Barra de Salvar fixa no rodapé. Não aparece como item próprio na sidebar.
