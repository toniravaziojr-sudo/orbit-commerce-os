...
**ERP:**
- `/fiscal` — Fiscal (abas: **Pedidos de Venda**, **Notas Fiscais**). Separadas por `fiscal_stage`: Pedidos de Venda lista `pedido_venda`; Notas Fiscais lista `pronta_emitir`, `pendencia` e `emitida`. **Aba Pedidos de Venda:** ação "Criar Nota Fiscal" executa `fiscal-prepare-invoice` (validação local, sem transmissão) e move o registro para Notas Fiscais. **Aba Notas Fiscais:** badges "Pronta para Emitir", "Pendência Identificada", "Processando", "Autorizada", "Rejeitada", "Cancelada". Ação "Enviar à Receita" (homologação: "Emitir NF-e de teste") é a única que transmite para SEFAZ via `fiscal-submit`/`fiscal-emit` com confirmação obrigatória. Pendência bloqueia envio; editor revalida automaticamente ao salvar. **Duplicar:** "Duplicar Pedido de Venda" mantém em Pedidos de Venda; "Duplicar NF" valida via `fiscal-prepare-invoice` e coloca em `pronta_emitir`/`pendencia` na aba Notas Fiscais — nunca volta para Pedidos de Venda. **DC-e oculta/desabilitada** no fluxo operacional comum até especificação completa de Fiscal/Logística. **Não confundir com o módulo `/orders` da loja**. Detalhe em `docs/especificacoes/erp/erp-fiscal.md`.
