...
**ERP:**
- `/fiscal` — Fiscal (abas: **Pedidos**, Notas Fiscais). Lista de Notas Fiscais inclui ações em massa **"Emitir DC-e"** (rascunhos com transportadora gateway) e **"Enviar à transportadora"** (NFes autorizadas com transportadora gateway — dispara `gateway-attach-fiscal-doc`). **Clonar Pedido / Clonar NF (rev 2026-05-14):** menu de ações em ambas as abas inclui item de clonagem (rótulo **"Clonar Pedido"** na aba Pedidos, **"Clonar NF"** na aba Notas Fiscais — disponível para rascunhos, autorizadas e canceladas). Gera sempre um rascunho independente (sem `order_id`, sem chave/XML/protocolo, número fiscal novo) — não emite, não chama provedor fiscal, não toca estoque/financeiro/remessa/e-mail/automação. Toast com ação "Abrir rascunho". Detalhe em `docs/especificacoes/erp/erp-fiscal.md`.
