...
**ERP:**
- `/fiscal` — Fiscal (abas: **Pedidos de Venda**, **Notas Fiscais**). Separadas por `fiscal_stage`: Pedidos de Venda lista `pedido_venda`; Notas Fiscais lista `pronta_emitir`, `pendencia` e `emitida`. **Aba Pedidos de Venda:** ação "Criar Nota Fiscal" (singular) / "Criar Notas Fiscais (N)" (plural) executa `fiscal-prepare-invoice` (validação local, sem transmissão) e move o registro para Notas Fiscais. Após criação, redirecionamento automático para a aba Notas Fiscais. **Declaração de Conteúdo dos Correios (rev 2026-05-15 — motor único):** disponível na aba Pedidos de Venda em duas modalidades — ação individual **"Gerar Declaração de Conteúdo"** no menu da linha e ação em massa **"Gerar Declarações de Conteúdo (N)"** na barra de seleção. Ambas abrem **modal obrigatório de responsabilidade** com: (a) aviso forte de que a Declaração de Conteúdo **não substitui Nota Fiscal** quando a emissão de NF-e for obrigatória; (b) **seleção obrigatória do motivo** em lista pronta (Venda/remessa sem NF-e por decisão do remetente · Devolução de consumidor final · Troca · Amostra sem valor comercial · Brinde · Bem pessoal · Outro). Selecionando "Outro", exibe campo de texto obrigatório para detalhar. O motivo escolhido é registrado no histórico e impresso no PDF. **No fluxo em massa, o mesmo motivo se aplica a todos os pedidos selecionados;** (c) checkbox obrigatório de aceite de responsabilidade do remetente. Sem motivo + checkbox marcados, o botão "Gerar" fica desabilitado. Ao confirmar, gera **PDF próprio não fiscal** (modelo A5 dos Correios, com cláusula LC 87/1996), com **numeração interna própria** (independente da numeração de NF-e), vinculado ao Pedido de Venda de origem. **Não altera `fiscal_stage`**, **não aparece na aba Notas Fiscais**, **não chama Focus/Sefaz**, **não gera XML, DANFE, chave de acesso nem protocolo SEFAZ**. O **gateway logístico** (Frenet e demais transportadoras) usa o **mesmo motor único** de Declaração de Conteúdo quando a remessa precisa partir sem NF-e — não existe caminho paralelo. Detalhe completo em `docs/especificacoes/fiscal/declaracao-de-conteudo-correios.md`. **Aba Notas Fiscais:** badges "Pronta para Emitir" (laranja), "Pendência Identificada" (amarelo), "Processando" (amarelo), "Autorizada" (azul), "Autorizada + Impressa" (azul + verde), "Rejeitada" (vermelho), "Cancelada" (vermelho), "Erro" (vermelho). Ação "Emitir Nota Fiscal" (homologação: "Emitir NF-e de teste") é a única que transmite para SEFAZ via `fiscal-submit`/`fiscal-emit` com confirmação obrigatória. Pendência bloqueia emissão; editor revalida automaticamente ao salvar. **Duplicar:** "Duplicar Pedido de Venda" mantém em Pedidos de Venda; "Duplicar NF" valida via `fiscal-prepare-invoice` e coloca em `pronta_emitir`/`pendencia` na aba Notas Fiscais — nunca volta para Pedidos de Venda. **Não confundir com o módulo `/orders` da loja**. Detalhe em `docs/especificacoes/erp/erp-fiscal.md`.
