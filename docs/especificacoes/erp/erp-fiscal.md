...
4. **Anti-duplicação via índice único (v2026-05-13 — Onda 1.A)**: O índice parcial `idx_fiscal_invoices_order_unique` em `(tenant_id, order_id) WHERE status NOT IN ('cancelled', 'rejected') AND order_id IS NOT NULL` impede a criação de múltiplos rascunhos/NFs ativas para o mesmo pedido. Notas `cancelled` e `rejected` (rejeitadas pela Sefaz) **não contam como ativas**, permitindo re-rascunho e reemissão após esses cenários. Conflitos no índice são tratados como "registro já existente" (fetch do invoice existente), não como erro.
   ...
   **Duplicar Pedido de Venda / Duplicar NF (v2026-05-14 — Onda 1.A rev3)**: A duplicação agora abre um diálogo pré-preenchido (`ManualInvoiceDialog` em `mode="duplicate"`) para o usuário revisar/editar antes de salvar; só ao clicar em **"Salvar duplicação"** o novo registro é criado.
   ...
   - Toast de sucesso: "Pedido de venda duplicado com sucesso." ou **"NF duplicada como rascunho em Pedidos de Venda. O editor abriu para revisão."**, com ação "Abrir registro duplicado" que abre o novo rascunho no editor completo. **Importante:** ao duplicar uma NF (autorizada/cancelada/rejeitada), o novo rascunho aparece na aba **Pedidos de Venda** (filtrada por `status='draft'`), não na aba Notas Fiscais — o editor abre automaticamente para evitar a sensação de "salvou e sumiu".
   ...
   **Emissão de NF-e — modal de confirmação (v2026-05-14 — Onda 1.A rev4)**: A ação "Emitir NF-e" no menu da linha de uma NF `draft` agora abre um modal de confirmação obrigatório (`AlertDialog`). Em ambiente `homologacao`, o rótulo do item vira **"Emitir NF-e de teste"** e o modal explica que a nota não terá valor fiscal real. Antes de habilitar o botão de confirmação, é executado um pré-check local (sem chamar Focus/Sefaz): configuração fiscal completa, CNPJ do certificado coincidindo com o emitente, destinatário com CPF/CNPJ válido, endereço completo, CEP válido, valor total > 0 e ausência de chave de acesso pré-existente. Pendências aparecem em lista vermelha e bloqueiam a confirmação. A transmissão para a SEFAZ só ocorre após o clique explícito em "Emitir NF-e (de teste)" no modal.

   **Status `processing` (v2026-05-14)**: Quando o Focus NFe retorna `processando_autorizacao`, a NF-e fica em `processing`. A lista exibe o badge **"Processando SEFAZ"** (Clock) e o menu da linha mostra **"Atualizar Status"** em vez de "Emitir" — assim o usuário não tenta reemitir uma nota que já está aguardando autorização.

   **Declaração de Conteúdo (DC-e) — indisponível nesta etapa (v2026-05-14)**: O botão **"Emitir DC-e"** na barra de ações em massa permanece visível para descoberta, mas exibe mensagem clara informando que a Declaração de Conteúdo ainda não está disponível para emissão automática. A integração com a transportadora será habilitada em onda futura. Não há transmissão real à Focus/Sefaz nesta etapa.
