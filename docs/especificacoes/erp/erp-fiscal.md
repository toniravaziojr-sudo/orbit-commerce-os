...
- Botão principal: **"Nova NF-e"** → cria rascunho **LIMPO** de NF Fiscal via `fiscal-create-manual` com `mode='nfe_manual'`. O rascunho nasce **sem destinatário pré-preenchido** e **sem item mockado**, com `fiscal_stage='pendencia'` (não `pedido_venda`). O **InvoiceEditor** abre em **modo NF Fiscal** (`isPedidoVenda=false`), com título "Série X – Nº Y", sem painéis de pendência/avisos pré-renderizados. Usuário escolhe natureza, destinatário e produto (via "Buscar produto"). Validações fiscais só disparam ao **Salvar/Emitir**, classificando para `pronta_emitir` ou `pendencia`. Este fluxo é **distinto** da criação manual de Pedido de Venda (aba Pedidos de Venda → "Novo Pedido de Venda" → `ManualInvoiceDialog` → `mode='pedido_venda'`). Ver memória anti-regressão `fiscal-manual-nf-vs-pedido-venda-separation`.
