...
| **Insights** | `insights` | `InsightsTab` | Cérebro Regenerativo v2.0 — aprendizados capturados em conversas reais, aprovados pelo admin e injetados nos 4 agentes de IA |
...
| 4 | **Integrações** | Plug | Meta token expirado/com erro, WhatsApp verificação pendente/desconectado, Email DNS pendente, **Rastreamento desativado** (transportadora ativa com `supports_quote = true` e `supports_tracking = false` — badge navega para `/shipping?tab=settings`) | `/integrations`, `/shipping?tab=settings` |
...
## 4. Aba Insights — Cérebro Regenerativo (v2.0)

**Componente:** `InsightsTab` (`src/components/command-center/InsightsTab.tsx`)  
**Hooks:** `useBrainInsights.ts`, `useBrainCriticalAlerts.ts`  
**Descrição:** Sistema de aprendizado contínuo que transforma conversas reais com clientes em regras operacionais que guiam os agentes de IA do sistema. Substitui o modelo antigo de "relatórios semanais estáticos".

> ⚠️ Alertas e insights **NÃO** aparecem na Central de Execuções. A Central é estritamente para itens que exigem ação humana.

### 4.1 Conceito do Cérebro Regenerativo

O sistema opera um ciclo fechado de 4 etapas que se repete continuamente:

```
1. CAPTURA          2. CONSOLIDAÇÃO       3. APROVAÇÃO          4. INJEÇÃO
   (evento real)       (cron semanal)        (humano)              (prompt)
   ─────────────────   ─────────────────     ─────────────────    ─────────────────
   Conversa            Sinais parecidos      Admin revisa e       Insight ativo entra
   finalizada →        são agrupados em      decide: aprovar,     no system prompt
   sinais são          candidatos com        descartar ou         dos agentes
   extraídos e         título, resumo e      editar escopo        selecionados
   armazenados         recomendação
```

**Benefício:** O que os agentes "sabem" sobre o negócio é construído a partir do que realmente acontece na loja — não de suposições genéricas. Cada aprendizado é aprovado explicitamente pelo dono antes de influenciar o comportamento da IA.

### 4.2 Sub-abas da Interface

| Sub-aba | Status exibido | Conteúdo | Ações disponíveis |
|---------|---------------|----------|-------------------|
| **Pendentes** | `pending` | Candidatos consolidados aguardando decisão humana. Exibe título, resumo, recomendação, número de ocorrências e clientes únicos | Aprovar (escolhendo escopo inicial) • Descartar (com motivo) |
| **Ativos** | `active` | Insights no cérebro dos agentes. Cada card mostra 4 toggles de escopo (Vendas, Auxiliar, Landing, Tráfego) | Alternar escopo por agente • Revogar (saída imediata do cérebro) |
| **Críticos** | tabela `ai_critical_alerts` | Alertas operacionais em tempo real (site fora, erro de checkout, falha de pagamento). Auto-refresh a cada 60s | Resolver • Ignorar |
| **Histórico** | `revoked` / `expired` | Log compacto de insights revogados ou expirados. Referência para auditoria | Somente leitura |

### 4.3 Escopo por Agente

Todo insight aprovado possui 4 flags independentes (`scope_vendas`, `scope_auxiliar`, `scope_landing`, `scope_trafego`). O admin decide em quais cérebros o aprendizado deve entrar. Exemplo:

- **"Cliente sempre pergunta sobre prazo de entrega antes de comprar"** → ativar em Vendas e Landing (não faz sentido em Tráfego).
- **"Faturamento cai às segundas-feiras"** → ativar apenas em Auxiliar (insight de gestão).

### 4.4 Automações (Cron Jobs)
...
As Edge Functions dos 4 agentes (`ai-support-chat`, `command-assistant-chat`, `ai-landing-page-generate`, `ads-autopilot-analyze` / `ads-autopilot-strategist`) leem a view `ai_brain_active_view` filtrando pelo escopo do agente e concatenam os aprendizados formatados no system prompt. Isso acontece automaticamente a cada chamada — nenhum cache intermediário.
...
| Regra | Descrição |
|-------|-----------|
| **Aprovação humana obrigatória** | Nenhum insight entra no cérebro sem clique explícito do admin. Não há aprovação automática. |
| **Revogação instantânea** | Ao revogar, o insight sai da view ativa no próximo ciclo de chamada dos agentes (sem cache). |
| **Expiração** | Insights com `expires_at` vencido saem automaticamente do cérebro e vão para Histórico. |
| **Tenant-scoped** | Cada tenant possui seu próprio cérebro. Insights nunca vazam entre tenants. |
| **Fallback** | Se a view retornar vazia, os agentes operam sem o bloco de aprendizados (nunca quebram). |
