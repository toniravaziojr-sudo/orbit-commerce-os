...
| 2026-05-05 | Limpeza do trigger legado `trg_recalc_customer_on_order` (BEFORE): removido bloco "ENRICH" duplicado que preenchia campos pessoais nulos. Enriquecimento do cadastro do cliente passa a ser responsabilidade exclusiva de `enrich_customer_from_order`. Trigger BEFORE mantém apenas: localizar/criar cliente por e-mail, vincular `customer_id` ao pedido e marcar `is_first_sale`. |
