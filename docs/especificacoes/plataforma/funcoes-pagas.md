...
| **meta-whatsapp-webhook** | Meta Cloud API (recepção) | **não cobra** — recepção pura, sem custo externo | Meta POST | resolve via `phone_number_id` | **D — não aplicável** | n/a |
| persistência inbound (`whatsapp_inbound_messages`, `conversations`, `messages`) | Postgres interno | **não cobra** — escrita interna | webhook | sim | **D — não aplicável** | n/a |
| **turn-orchestrator-processor** | — (orquestrador) | **não cobra** — herda cobrança do `ai-support-chat` (evita dupla cobrança) | webhook (fetch interno) | sim | **D — não aplicável** | n/a |
| **agenda-process-command** | Gemini 2.5 Flash via Lovable AI Gateway (agente Agenda do tenant) | **cobra IA do tenant** via `chargeAfter` (F2.13.1) | webhook (telefone admin) | sim | **B — chargeAfter ATIVO** | baixo (cobrança plugada, idempotência por `external_message_id`) |

**F2.13 (2026-05-11):** auditado o fluxo de recepção WhatsApp/Meta. **Não existe `whatsapp-webhook` separado** — `meta-whatsapp-webhook` é a única edge de recepção. Recepção, dedupe, persistência inbound (`whatsapp_inbound_messages`, `conversations`, `messages`) e orquestração (`turn-orchestrator-processor`) classificadas como **D — não aplicáveis**. `ai-support-chat` reafirmada como ponto correto de cobrança da IA de atendimento. `meta-whatsapp-send` reafirmado como D para custo Meta. Riscos de PII em logs do webhook → backlog **F2.13.2**. Ver §20 do doc F2.

**F2.13.1 (2026-05-11):** plugada cobrança de IA da Agenda em `agenda-process-command` via `chargeAfter` pós-sucesso da chamada Gemini. Captura `usage` real do Lovable AI Gateway (`prompt_tokens`/`completion_tokens`); cobra `gemini.gemini-2.5-flash.per_1m_tokens_in` e `gemini.gemini-2.5-flash.per_1m_tokens_out` (pricing existente, sem criar service_key novo). `jobId` determinístico `agenda:${external_message_id}:in|out` garante idempotência em redelivery do webhook (somando ao dedupe de `agenda_command_log`). Não cobra se provider IA falhar; não cobra se `usage` vier ausente/inválido (log sanitizado). Cobra mesmo se ação interna posterior falhar (custo IA já consumido). `intent=delegate_to_assistant` cobra apenas a IA da Agenda — `command-assistant-execute` não chama IA hoje. Sem alteração em `meta-whatsapp-send`, `meta-whatsapp-webhook`, `ai-support-chat`, `turn-orchestrator-processor`, `command-assistant-execute` ou UI/UX. Metadata sanitizada (sem `from_phone`/`message_content`/prompt/resposta/PII bruta). Ver §21 do doc F2.
