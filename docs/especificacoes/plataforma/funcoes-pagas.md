...
| **agenda-process-command** | Gemini (agente Agenda do tenant) | **deve cobrar IA do tenant** — pendente F2.13.1 | webhook (telefone admin) | sim | **B — chargeAfter (pendente)** | médio (custo IA invisível hoje) |

**F2.13 (2026-05-11):** auditado o fluxo de recepção WhatsApp/Meta. **Não existe `whatsapp-webhook` separado** — `meta-whatsapp-webhook` é a única edge de recepção. Recepção, dedupe, persistência inbound (`whatsapp_inbound_messages`, `conversations`, `messages`) e orquestração (`turn-orchestrator-processor`) classificadas como **D — não aplicáveis** ao Motor de Créditos. `ai-support-chat` reafirmada como ponto correto de cobrança da IA de atendimento. `meta-whatsapp-send` reafirmado como D para custo Meta (cliente paga direto à Meta). **`agenda-process-command` é agente de IA do tenant** (conversa com o tenant no WhatsApp, executa ações no sistema e dialoga com outros agentes de IA), portanto seu custo de IA **deve** ser cobrado do tenant via Motor de Créditos — auditoria e implementação pendentes em **F2.13.1**. Riscos de PII em logs do webhook (telefone, wa_id, texto, profile, IDs Meta) catalogados como backlog **F2.13.2** sem correção nesta entrega. Ver §20 do doc F2.
