...
> **Status:** 🟩 Atualizado  
> **Última atualização:** 2026-06-29 (v3.13: **Paridade total do pedido ML com o pedido da loja.** `meli-sync-orders` agora aloca `order_number` canônico via `generate_order_number(tenant_id)` (mesma sequência #NNN da loja); o ID do ML permanece em `marketplace_order_id`/`source_order_number`/`marketplace_data.meli_order_id` para rastreabilidade. `is_first_sale` é calculado consultando se o cliente já tem outro pedido no tenant — evita "1ª Venda" no 2º pedido do mesmo comprador. Toda criação grava `order_history` com a origem ("Pedido importado do Mercado Livre"). Re-sync **preserva** o número alocado e o flag de primeira venda (só atualiza dados mutáveis). UI do badge: para `mercadolivre` o handshake oficial é renderizado sem o wrapper circular extra (a logo já traz o círculo amarelo embutido), eliminando a moldura duplicada. Backfill aplicado aos pedidos #662 e #663 do tenant respeiteohomem.)
> **Histórico v2.6.0 (2026-06-29):** Garantia fonte única + Edit síncrono robusto + Anti-regen no Edit Dialog.
> 1. `meli-publish-listing` (publish e update) **remove qualquer `WARRANTY_TYPE`/`WARRANTY_TIME` herdado** do painel/IA/memória e reinjeta a partir de `products.warranty_type`/`warranty_duration`. Se o cadastro estiver `none/vazio`, ambos são omitidos (ML mostra "Sem garantia" por default). `WARRANTY_TIME` foi adicionado ao set de campos free-form do sanitizador para não ser bloqueado pela lista fechada da categoria.
> 2. O sanitizador de update (`sanitizeAttributesForCategory`) passou a suportar `values[]` (multi-valor) cruzando com a lista oficial; o handler de update passou a usar `humanizeMeliError` e a persistir `error_message` em `meli_listings` quando o ML rejeita o PUT.
> 3. `MeliAttributesPanel` faz **self-healing silencioso**: rascunhos/anúncios com características salvas para a mesma categoria são reaproveitados mesmo sem `resolver_version`, eliminando a regeneração automática ao reabrir o Edit Dialog. O cache é re-selado com a versão atual em segundo plano. `MeliListingCreator.handleSaveAttributes` e `MeliListingWizard.handleSubmit` passaram a persistir o payload completo (name/source/status/required/resolver_version/values/not_applicable).)
> **Histórico v1.15.0 (2026-06-29):** padronização de `product_type` para termos funcionais (ex: "Balm capilar", "Loção capilar") nos kits de quantidade do tenant `respeiteohomem`; selos `MarketplaceFieldHint` nos campos críticos do `ProductForm`; e diagnóstico de falha de categorização (`diagnoseCategoryFailure`) com motivo + ação + link direto para o cadastro. Adoção da Regra Universal §36 (Guiar o Usuário) no módulo ML.
> **Histórico v2.5.0 (2026-06-28):** alinhamento de **Frete Grátis** com a regra do Mercado Livre Brasil (preço ≥ R$ 79 → `free_shipping=true` forçado + persistência do `shipping` real após publicação; webhook/sync reconciliam passivamente).
> **Histórico v2.4.4 (2026-06-29):** **Normalizador ANVISA no adapter.** `meli-publish-listing` (publish e update) passa por `normalizeAnvisaNumber` (em `_shared/marketplace-adapter/meli/regulatory-normalizer.ts`) antes de enviar o número ao ML. Cadastro pode persistir só dígitos (17 = Notificação/Comunicação prévia → `XXXXX.XXXXXX/XXXX-XX`; 13 = Registro de produto → `X.XXXX.XXXX.XXX-X`) ou já formatado; o adapter formata sem alterar o cadastro. Valor inválido = atributo omitido. Cobre o erro `item.attribute.invalid_sanitary_registry_value` que rejeitou Fast Upgrade/Loção Noite. `humanizeMeliError` explica os dois formatos aceitos.
> **Histórico v2.4.3 (2026-06-27):** adaptador injeta `UNITS_PER_PACK=1` quando ausente; drop universal de AFE/CONAMA/ANVISA-number quando cadastro vazio.
