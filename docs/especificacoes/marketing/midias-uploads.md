...
> **Migrado de:** `docs/regras/midias-uploads.md`  
...
| 2026-03-28 | **Scroll fix v2** — Correção definitiva do scroll em todos os modais do calendário de conteúdo (PublicationDialog, ApprovalDialog, DayPostsList, CalendarItemDialog, PublicationPreviewDialog). O `DialogContent` do shadcn usa `display: grid` internamente, que não é sobrescrito por classes Tailwind `flex`. Solução: `style={{ display: 'flex', flexDirection: 'column', gap: 0 }}` inline em todos os DialogContent que precisam de scroll interno. Chain flex: `form` e `fieldset` com `overflow-hidden`, div scrollável com `flex-1 overflow-y-auto min-h-0`. |
