...
> **v2026-06-30 — Tarja universal de cancelamento (loja + marketplace):** todo pedido cancelado exibe uma linha discreta (`BuyerCancellationNotice`) abaixo do status nas três telas principais — **`/orders`** (Pedidos), **`/fiscal`** aba **Pedidos de Venda** e **`/fiscal`** aba **Notas Fiscais**. O texto reflete a origem real do cancelamento sem necessidade de coluna adicional:
> - `orders.status='cancelled_by_user'` → **"Pedido cancelado pelo lojista"** (cancelamento manual no Comando Central).
> - `orders.status='cancelled'` + `cancellation_reason` casando regex de comprador/mediação/expiração/chargeback → texto específico (ex.: "Pedido cancelado pelo comprador", "Pedido cancelado em mediação no Mercado Livre").
> - `orders.status='cancelled'` sem motivo reconhecível → **"Pedido cancelado"** genérico.
> - `orders.status='chargeback_lost'` → **"Pedido cancelado por chargeback"**.
>
> Na tela `/fiscal` aba Pedidos de Venda, a condição da tarja é `pedidoStatusOf(invoice) === 'cancelado'` (lê `pedido_status` espelhado pelo trigger `trg_orders_sync_pv_status`), porque a coluna `fiscal_invoices.status` permanece `draft` mesmo após o pedido ser cancelado — quem reflete o cancelamento no PV é `pedido_status`. Na aba Notas Fiscais, continua sendo `invoice.status === 'cancelled'`. Os dados são carregados em batch (um único `SELECT` por página) para evitar N+1.
