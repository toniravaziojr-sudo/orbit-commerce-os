...
| 2026-03-10 | **NEWSLETTER POPUP EXIT INTENT FIX**: Corrigido loop infinito do popup com trigger `exit_intent`. Mudanças: (1) Flag `eiFired` impede re-trigger após exibição ou dismiss. (2) Detecção melhorada: `mouseleave` (cursor saiu do viewport) + `mousemove` com `clientY < 50` e `movementY < -5` (movimento rápido em direção ao botão fechar). (3) Delay de 2.5s antes de ativar listener para evitar falso positivo no load. (4) Dispara no máximo UMA vez por sessão. Aplicado em: Edge HTML (`storefront-html`) e SPA React (`NewsletterPopupBlock.tsx`). |
