...
| 2026-03-10 | **SUPPORT WIDGET STOREFRONT (v1.0)**: Widget flutuante de atendimento na loja pública. Configurável via Builder (Atendimento → ThemeSettings). Suporta 3 modos: Chat interno, WhatsApp, ou Ambos. Edge Function `storefront-html` injeta HTML do widget via `generateSupportWidgetHtml()` antes de `</body>`. Componente SPA: `SupportChatWidget.tsx`. Configuração: `themeSettings.supportWidget` (enabled, type, whatsappNumber, whatsappMessage, buttonColor, position). Oculto automaticamente em rotas `/checkout`. **Lead Capture (v1.1)**: Ao preencher nome/email no chat, o lead é registrado automaticamente via `marketing-form-submit` com `source: 'support_chat'`. A lista "Leads site" (slug: `leads-site`) é auto-criada se não existir, com tag vinculada. Ícone WhatsApp: SVG oficial (não telefone). |
