...
14. **Identidade do cliente unificada** — sync de pedido marketplace busca cliente existente por `external_id → cpf → cnpj → email_real → phone (variantes com/sem 55)` antes de criar novo. Quando o cliente já tem e-mail/telefone reais, esses dados são reusados em `orders.customer_email`/`customer_phone` para que notificações funcionem. E-mail sintético (`@marketplace.local`, `@shopee.user`, `@tiktok.user`) só persiste para satisfazer a constraint NOT NULL e nunca aparece em UI (helper `displayCustomerEmail` mostra "Sem e-mail informado").
15. **Automação fiscal e logística unificada** — pedidos marketplace seguem a MESMA flag `fiscal_settings.emissao_automatica` da loja. Logística externa (envio de chave NF ao marketplace + download de etiqueta) é sempre automática em resposta ao evento `nf_authorized`. Sem flag exclusiva por marketplace.
