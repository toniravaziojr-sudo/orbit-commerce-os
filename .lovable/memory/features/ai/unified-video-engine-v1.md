...
## Motor Único de Geração de Vídeo (v1.1)
...
| `creative_vfx` | mascote, personagem, animação, efeito, VFX, cinematic, fantasia, neon, holográfico, 3D, glitch, fogo, fumaça, raio | Veo 3.1 |
...
2. **Veo 3.1** (fal.ai) — `fal-ai/veo3.1` ou `fal-ai/veo3.1/reference-to-video`. Áudio nativo + melhor para VFX/mascote/animação. Fallback: Kling → Wan.
...
- `_shared/visual-engine.ts` — Motor único de imagens com `resilientGenerate()` e `classifyImageIntent()`
