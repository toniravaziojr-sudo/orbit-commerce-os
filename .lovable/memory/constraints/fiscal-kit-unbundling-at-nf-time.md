...
5. Rateio proporcional ao preço de venda de cada componente; **`valor_total` de cada componente é sempre `round2(quantidade × valor_unitario)` para evitar rejeição Sefaz NA01 ("Total do produto difere de Qtd × Vlr Unit")**; diferença de centavos é absorvida ajustando o `valor_unitario` do último componente para preservar exatamente `valor_total` da NF.
