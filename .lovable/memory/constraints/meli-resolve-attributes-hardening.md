...
14. **Cobertura total da categoria + "Não se aplica" (v1.9.0).** Toda categoria escolhida na Etapa 1 é fonte única. O resolvedor cobre 100% dos atributos da categoria com 3 resultados possíveis: (a) valor real (cadastro/derivação/dicionário/IA com certeza); (b) `not_applicable: true` para opcionais sem valor OU quando IA devolve `"NAO_SE_APLICA"`; (c) `status: "missing"` apenas para obrigatórios sem valor confiável (bloqueia publicação). Proibido omitir atributos opcionais — eles DEVEM ir ao ML com o marcador oficial "Não se aplica" (busca opção `"Não se aplica"`/`"N/A"` na lista da categoria, ou fallback `{value_id:"-1", value_name:"N/A"}`). WARRANTY_TYPE tem fallback `"Sem garantia"` quando cadastro vazio. Sanitização do publish substitui valores fora da lista oficial pelo marcador N/A.

15. **Cadastro completo como ficha primária + números regulatórios automáticos (v2.0.0).** (a) SELECT do resolver DEVE incluir `regulatory_info`, `regulatory_category`, `product_type`, `ai_product_type`, `ai_main_function`, `line`, `model`, `description`, `short_description` — esses campos compõem a ficha primária e são enviados na íntegra ao prompt da IA (descrição longa truncada em 1500 chars). (b) Atributos da categoria do ML cujo NOME (não ID) contenha "Anvisa + Número/Notificação/Comunicação/Registro/Documento" são preenchidos automaticamente com `products.regulatory_info.anvisa`; idem para AFE (`afe + certificado/numero/autorizac`) e CONAMA. Match por nome é obrigatório porque IDs variam por categoria do ML (`ANVISA_PRIOR_NOTIFICATION_COMMUNICATION_DOCUMENT_NUMBER`, `ANVISA_PRODUCT_REGISTRATION_NUMBER` etc.). Implementado em paralelo no resolver e no `meli-publish-listing`. (c) Fallback determinístico por tokens entre cadastro e lista oficial do ML passa a rodar também em opcionais (score mínimo 2 para opcional, 1 para obrigatório). (d) Limite conhecido: se o tipo do produto não tem equivalente na lista oficial da categoria escolhida (ex.: Shampoo em categoria de Tratamentos Capilares), o motor não inventa — marca "Não se aplica" e a correção é trocar a categoria do anúncio. Proibido inventar opção fora da lista para "salvar" o atributo.

Docs: `docs/especificacoes/marketplaces/mercado-livre.md` seções v1.5.0 a v1.9.0 e **v2.0.0 — Cadastro Completo como Ficha Primária + Números Regulatórios**.
