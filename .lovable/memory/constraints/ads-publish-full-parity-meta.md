...
## Regras críticas adicionadas em 2026-06-19 (v1.5.0 → v1.9.0)

- **Registro da lista de "clientes atuais" no NÍVEL DA CONTA (v1.9.0 — causa raiz):** antes de publicar campanha com `is_new_customer_acquisition=true`, o publicador OBRIGATORIAMENTE registra a audiência de Clientes em `existing_customers` da conta de anúncios via `POST /act_<id>` (idempotente: lê `GET /act_<id>?fields=existing_customers`, só escreve se a audiência não estiver na lista). Esse cadastro é o pré-requisito **documentado pela Meta** (https://developers.facebook.com/docs/marketing-api/advantage-shopping-campaigns/#step-1) para a flag `is_new_customer_acquisition` engatar — sem ele, a Meta aceita o POST mas retorna `is_new_customer_acquisition=false` na leitura, mesmo com `customer_acquisition_spec` correto e sem conflitos de exclusão. Reproduzido em produção em 2026-06-19 (campanha "Shampoo Calvície Zero", proposta `ee2cf98d` — falhou v1.7.0 e v1.8.0 mesmo após dedupe de exclusão). Falha em registrar a lista bloqueia a publicação com mensagem PT-BR ("Não foi possível registrar a sua lista de clientes na conta de anúncios da Meta…"). PROIBIDO publicar campanha de aquisição sem garantir esse registro.
