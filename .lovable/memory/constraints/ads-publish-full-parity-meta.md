...
## Regras críticas adicionadas em 2026-06-19 (v1.5.0 → v1.6.0)
...
- **Estratégia de ciclo de vida com pré-requisito (v1.6.0):** Meta só ativa `is_new_customer_acquisition=true` se a campanha também trouxer `customer_acquisition_spec.custom_audiences` apontando para uma audiência de "clientes atuais" existente na conta. O publicador busca no catálogo de audiências da conta uma que case com regex `/compradores?|clientes?|customers?|buyers?|purchas|compra|comprou/i`; se achar, anexa automaticamente; se não achar, publica sem a flag e grava `lifecycle_notice_pt` na proposta com a instrução de criar a audiência (Gerenciador de Anúncios → Públicos → evento Purchase). Após criar a campanha, lê de volta `GET /<campaign_id>?fields=is_new_customer_acquisition` e grava resultado em `parity_check.lifecycle`. PROIBIDO declarar ciclo de vida aplicado sem confirmação via leitura de retorno da Meta.
