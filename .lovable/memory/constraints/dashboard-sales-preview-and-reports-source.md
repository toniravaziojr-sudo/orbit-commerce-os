...
- Fonte única de "venda realizada": pedidos com status em `('paid','processing','ready_to_invoice','invoiced','shipped','delivered')` no período em BRT. `ready_to_invoice` e `invoiced` são pós-pagamento e DEVEM contar como venda — omiti-los esconde 100% das vendas em lojas que avançam o pedido direto para emissão fiscal após o pagamento. Mesma regra no dashboard e nos relatórios.
