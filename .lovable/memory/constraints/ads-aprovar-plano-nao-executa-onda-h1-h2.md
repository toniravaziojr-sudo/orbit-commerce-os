...
## Renderização de campaign_proposal (UI)
A `campaign_proposal` grava `action_data.schema_version = "campaign_proposal_v1"` com `campaign{}`, `adsets[]`, `planned_creatives[]` e `raw_planned_action`. A UI lê via `normalizeCampaignStructure` (rama `fromCampaignProposalV1`), que cruza o dialeto H.2 com `raw_planned_action.adsets[i]` para preencher idade/gênero/local/orçamento/audience_type/optimization_goal/conversion_event quando vierem só no raw. **Não criar tela nova**: usa o `StructuredProposalModal` existente. Botão Aprovar fica desabilitado tanto no card quanto no modal (mensagem PT-BR de próxima etapa) até a Onda H.3.
