...
7. **Aprendizado nasce ATIVO por padrão (Onda 3.2 — 2026-06-17)**: tanto `ads-ai-learnings-write` quanto o fallback direto em `ads-autopilot-feedback-record` devem inserir novos registros em `ads_ai_learnings` com `status = 'active'`, independente da origem (`manual`, `adjustment`, `rejection`, `approval`). Aprendizado entrando como `suggested` por padrão é regressão — o usuário não deve precisar ativar manualmente para a IA passar a considerar. UI deve refletir: "novo aprendizado já entra ativo; pause/edite se quiser".
