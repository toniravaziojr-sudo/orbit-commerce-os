...
4. **Status final — Reg #2.13 Fase C:** `complete_turn` SOMENTE após envio aceito (`meta-whatsapp-send.success === true` para WhatsApp, `success:true` p/ chat/email, ou `dry_run:true` em sandbox). Falha de envio → `fail_turn` com mesmo `bot_message_id` (retry idempotente reutiliza a mesma mensagem; índice único impede 2ª resposta). Sandbox NUNCA chama Meta sem allowlist + `x-allow-real-send: true` + `recipient_override` — ver `mem://constraints/sandbox-real-meta-leak`.
