...
export default function EmailMarketingListDetail() {
  ...
  const { data: totalCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      if (!listId || !list?.tag_id) return 0;
      
      // Get customer_ids from tag assignments
      const { data: tagAssignments } = await supabase
        .from("customer_tag_assignments")
        .select("customer_id")
        .eq("tag_id", list.tag_id);
      
      if (!tagAssignments?.length) return 0;
      
      const customerIds = tagAssignments.map(a => a.customer_id);
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      if (!listId || !list?.tag_id) return { active: 0, unsubscribed: 0, bounced: 0 };
      ...
      if (!tagAssignments?.length) return { active: 0, unsubscribed: 0, bounced: 0 };
