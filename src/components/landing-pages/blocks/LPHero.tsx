...
export function LPHero({ data }: Props) {
  ...
  // Background style: scene > stock background > gradient fallback
  ...
  return (
    <section
      className={`relative overflow-hidden lp-noise ${hasScene ? 'lp-hero-scene' : ''}`}
      style={{ ...bgStyle, minHeight: hasScene ? '700px' : undefined }}
    >
      {/* Mobile scene swap */}
      ...
      {/* 2-column layout: text LEFT, product RIGHT (always) */}
      <div 
        className="relative grid grid-cols-1 md:grid-cols-2 items-center gap-8 md:gap-16 px-[5%] py-24 md:py-32 max-w-[1140px] mx-auto"
        style={{ minHeight: hasScene ? '700px' : undefined }}
      >
        {/* Text column — always LEFT on desktop */}
        ...
        {/* Product column — ALWAYS rendered as real packshot overlay */}
        <div className="flex items-center justify-center order-first md:order-last">
          {hasProduct && (
            <div className="relative lp-hero-title-enter flex items-center justify-center" style={{ animationDelay: '0.3s' }}>
              ...
              <div 
                className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[70%] h-[20px] rounded-full blur-[15px] opacity-[0.3]"
              ...
              <img
                ...
                style={{ 
                  ...
                  maxWidth: 'clamp(240px, 28vw, 360px)',
                  maxHeight: 'clamp(280px, 42vh, 480px)',
                }}
              />
            </div>
          )}
