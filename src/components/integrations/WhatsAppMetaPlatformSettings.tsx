...
export function WhatsAppMetaPlatformSettings() {
  ...
  const saveMutation = useMutation({
    ...
    onError: (error) => showErrorToast(error, { module: 'integrações', action: 'salvar' }),
  });
  ...
  const testSendMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.success) {
        ...
        showErrorToast(new Error(data?.error || 'Falha ao enviar mensagem de teste'), { module: 'integrações', action: 'enviar' });
      }
    },
    onError: (error: any) => {
