// Gestão de Criativos — Exports

// Aba unificada de vídeos (nova)
export { UnifiedVideoTab } from './UnifiedVideoTab';

// Abas legadas (mantidas para compatibilidade, mas não mais usadas diretamente)
export { UGCClientTab } from './UGCClientTab';
export { UGCAITab } from './UGCAITab';
export { ProductVideoTab } from './ProductVideoTab';
export { ProductImageTab } from './ProductImageTab';
export { AvatarMascotTab } from './AvatarMascotTab';

// Componentes compartilhados
export { CreativeJobsList } from './CreativeJobsList';
export { CreativeGallery } from './CreativeGallery';
export { VoiceSelector } from './VoiceSelector';
export { VoicePresetManager } from './VoicePresetManager';
export { VideoJobsList } from './VideoJobsList';

// Sub-formulários de vídeo
export * from './video-forms';