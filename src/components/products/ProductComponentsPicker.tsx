// =============================================
// PRODUCT COMPONENTS PICKER - Works with local state for new products (kits)
// =============================================
...
interface ProductComponentsPickerProps {
  ...
  onComponentsChange: (components: PendingComponent[]) => void;
...
export function ProductComponentsPicker({ 
  components,
  onComponentsChange,
  stockType, 
  onStockTypeChange,
  excludeProductId,
}: ProductComponentsPickerProps) {
  ...
  const virtualStock = useMemo(() => {
    ...
    return Math.floor(Math.min(
      ...components.map(c => (c.component?.stock_quantity ?? 0) / c.quantity)
    ));
  }, [components]);
  ...
  }, [searchValue, profile?.current_tenant_id, excludeProductId, components]);

  const handleAddComponent = (product: ProductOption) => {
    const newComponent: PendingComponent = {
      id: `pending-comp-${Date.now()}-${Math.random().toString(36).substring(7)}`,
  ...
  const handleCostPriceChange = (componentId: string, newPrice: number | null) => {
    onComponentsChange(components.map(comp => 
      comp.id === componentId ? { ...comp, cost_price: newPrice } : comp
    ));
  };

  const handleSalePriceChange = (componentId: string, newPrice: number | null) => {
    onComponentsChange(components.map(comp => 
      comp.id === componentId ? { ...comp, sale_price: newPrice } : comp
    ));
  };
                      ...
                      <TableRow key={component.id}>
                        ...
                        <TableCell>
                          <Input
                            ...
                            onChange={(e) => handleQuantityChange(component.id, parseFloat(e.target.value) || 1)}
