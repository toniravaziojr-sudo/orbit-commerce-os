// =============================================
// CUSTOM SHIPPING RULE DIALOG
// Dialog for creating/editing custom shipping rules with pricing
// =============================================
...
import { type ShippingCustomRule, normalizeCep } from '@/hooks/useShippingRules';
...
interface CustomShippingRuleDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  rule: ShippingCustomRule | null;
  onSave: (data: Omit<ShippingCustomRule, 'id' | 'tenant_id' | 'created_at' | 'updated_at'>) => void;
  isLoading?: boolean;
}

export function CustomShippingRuleDialog({
  open,
  onOpenChange,
  rule,
  onSave,
  isLoading,
}: CustomShippingRuleDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {rule ? 'Editar Regra de Frete Personalizado' : 'Nova Regra de Frete Personalizado'}
          </DialogTitle>
          <DialogDescription>
            Configure uma regra de frete com preço fixo por região e faixa de CEP
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Ex: Capital SP - Econômico" {...field} />
                  </FormControl>
                  <FormDescription>
                    Este nome aparece como opção de frete no checkout
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Preço do Frete (R$)</FormLabel>
                  <FormControl>
                    <Input 
                      type="number"
                      step="0.01"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Esta regra só aplica se o pedido for maior ou igual a este valor
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Prioridade / Ordenação</FormLabel>
                  <FormControl>
                    <Input 
                      type="number"
                      min={0}
                  ...
                  <FormDescription>
                    Define a ordem de exibição no checkout (menor número = aparece primeiro)
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
