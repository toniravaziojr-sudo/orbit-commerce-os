// =============================================================================
// UTM — modelo interno fixo de produção (Onda F).
// Aplica utm_source/medium/campaign/content/term ao link final do anúncio,
// preservando query params existentes e SEM sobrescrever utm_* já definidas.
// Quando há conflito de UTM, registra warning técnico e mantém o valor original.
// =============================================================================
...
/**
 * Aplica modelo padrão de UTM preservando o que já existir no link.
 * - Mantém todos os query params existentes.
 * - Para cada utm_* obrigatória, só insere se ausente.
 * - Se já houver utm_* preenchida pelo usuário/IA, mantém e adiciona warning.
 */
export function applyUtm(rawUrl: string, input: UtmInput): UtmResult {
  ...
  for (const [key, val] of Object.entries(desired)) {
    ...
    if (existing && existing.trim() !== "") {
      if (existing !== val) {
        warnings.push(`utm_conflict:${key}:kept_existing`);
      }
