...
-- =====================================================================
-- Onda 2.A — Central de SEO: fundação read-only
-- Todas as views usam security_invoker=true (PG15+). RLS das tabelas
-- originais continua sendo aplicada no papel do caller.
-- Nenhuma tabela/coluna/trigger/cron/cache é criado.
-- =====================================================================

-- 1) Produtos ativos --------------------------------------------------
CREATE OR REPLACE VIEW public.v_seo_coverage_products
...
SELECT
  ...
  length(coalesce(p.seo_title,'')) AS title_len,
  length(coalesce(p.seo_description,'')) AS description_len
FROM public.products p
WHERE p.status = 'active'
  AND p.deleted_at IS NULL;

-- 2) Categorias ativas -----------------------------------------------
CREATE OR REPLACE VIEW public.v_seo_coverage_categories
WITH (security_invoker = true) AS
SELECT
  c.tenant_id,
  c.id,
  ...
  length(coalesce(c.seo_title,'')) AS title_len,
  length(coalesce(c.seo_description,'')) AS description_len
FROM public.categories c
WHERE c.is_active = true;

-- 3) Páginas do storefront publicadas e indexáveis
--    (exclui type='landing_page' para não duplicar com a view de LPs)
CREATE OR REPLACE VIEW public.v_seo_coverage_storefront_pages
WITH (security_invoker = true) AS
SELECT
  sp.tenant_id,
  sp.id,
  ...
  (COALESCE(sp.meta_title, sp.seo_title) IS NOT NULL
     AND length(btrim(COALESCE(sp.meta_title, sp.seo_title))) > 0)              AS has_title,
  (COALESCE(sp.meta_description, sp.seo_description) IS NOT NULL
     AND length(btrim(COALESCE(sp.meta_description, sp.seo_description))) > 0)  AS has_description,
  length(coalesce(COALESCE(sp.meta_title, sp.seo_title),''))              AS title_len,
  length(coalesce(COALESCE(sp.meta_description, sp.seo_description),'')) AS description_len,
  (sp.canonical_url IS NOT NULL AND length(btrim(sp.canonical_url)) > 0) AS has_canonical
...
-- 4) Landing pages indexáveis ----------------------------------------
CREATE OR REPLACE VIEW public.v_seo_coverage_landing_pages
WITH (security_invoker = true) AS
SELECT
  sp.tenant_id,
  sp.id,
...
CREATE OR REPLACE VIEW public.v_seo_coverage_blog_posts
WITH (security_invoker = true) AS
SELECT
  bp.tenant_id,
  bp.id,
  ...
  length(coalesce(bp.seo_title,'')) AS title_len,
  length(coalesce(bp.seo_description,'')) AS description_len
FROM public.blog_posts bp
WHERE bp.status = 'published';

-- 6) Fundação técnica local da loja ----------------------------------
--    (informativo — Onda 1 é fonte de verdade em storefront/seo.md)
CREATE OR REPLACE VIEW public.v_seo_store_foundation
WITH (security_invoker = true) AS
SELECT
  ss.tenant_id,
  (ss.favicon_url IS NOT NULL OR ss.favicon_file_id IS NOT NULL) AS has_favicon,
  (ss.logo_url IS NOT NULL OR ss.logo_file_id IS NOT NULL)       AS has_logo,
  ...
  ss.is_published AS store_published,
  (
    SELECT bool_or(td.verified_at IS NOT NULL AND td.is_primary = true)
    FROM public.tenant_domains td
    WHERE td.tenant_id = ss.tenant_id
  ) AS has_primary_verified_domain,
  (
    SELECT bool_or(COALESCE(td.ssl_status,'') IN ('active','issued','ready'))
    FROM public.tenant_domains td
    WHERE td.tenant_id = ss.tenant_id
  ) AS has_active_ssl
...
-- =====================================================================
-- Função: Índice de Saúde SEO (on-demand, tenant-scoped)
-- =====================================================================
...
BEGIN
  ...
  SELECT count(*), count(*) FILTER (WHERE has_title AND has_description AND NOT no_index)
    ...
    FROM v_seo_coverage_landing_pages WHERE tenant_id = p_tenant_id AND NOT no_index;

  SELECT count(*) INTO v_lp_noindex
    FROM v_seo_coverage_landing_pages WHERE tenant_id = p_tenant_id AND no_index;

  SELECT count(*), count(*) FILTER (WHERE has_title AND has_description)
    INTO v_bp_total, v_bp_ok
    FROM v_seo_coverage_blog_posts WHERE tenant_id = p_tenant_id;

  SELECT * INTO v_found FROM v_seo_store_foundation WHERE tenant_id = p_tenant_id LIMIT 1;

  -- Títulos duplicados (produtos+categorias+pages+lps+blog) por tenant
  SELECT count(*) INTO v_dup_titles FROM (
    SELECT lower(btrim(t)) AS k, count(*) c FROM (
      SELECT seo_title AS t FROM v_seo_coverage_products     WHERE tenant_id = p_tenant_id AND has_title
      UNION ALL SELECT seo_title FROM v_seo_coverage_categories        WHERE tenant_id = p_tenant_id AND has_title
      UNION ALL SELECT effective_title FROM v_seo_coverage_storefront_pages WHERE tenant_id = p_tenant_id AND has_title
      UNION ALL SELECT effective_title FROM v_seo_coverage_landing_pages    WHERE tenant_id = p_tenant_id AND has_title AND NOT no_index
      UNION ALL SELECT seo_title FROM v_seo_coverage_blog_posts       WHERE tenant_id = p_tenant_id AND has_title
    ) u
    GROUP BY 1
    HAVING count(*) > 1
  ) d;

  -- Cálculo do score (cada bloco pondera pela cobertura da entidade)
  ...
  -- Fundação técnica (5 sinais, peso 20)
  ...
  ELSE
    f_score := w_foundation * (
      ...
      (CASE WHEN v_found.has_primary_verified_domain THEN 1 ELSE 0 END) +
  ...
  v_factors := v_factors || jsonb_build_object(
    ...
    'signals', jsonb_build_object(
      ...
      'primary_verified_domain', COALESCE(v_found.has_primary_verified_domain,false),
...
-- Onda 4.1: default revoke ativo. Conceder explicitamente para authenticated.
REVOKE ALL ON FUNCTION public.calc_seo_health(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.calc_seo_health(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.calc_seo_health(uuid) TO service_role;

COMMENT ON FUNCTION public.calc_seo_health(uuid) IS
  'Onda 2.A — Índice de Saúde SEO on-demand. STABLE, SECURITY INVOKER, tenant-scoped via user_has_tenant_access + RLS das views.';
