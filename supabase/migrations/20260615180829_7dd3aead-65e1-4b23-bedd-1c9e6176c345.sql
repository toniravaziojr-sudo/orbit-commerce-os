...
-- =============================================================================
-- Onda H.2.1 — Função idempotente de patch para campaign_proposal v1.1
--
-- Replica a derivação determinística de src/lib/ads/contracts/campaignProposalV1_1.ts
-- em PL/pgSQL. Não cria defaults, não chama Meta/IA, não toca em status/lifecycle.
-- =============================================================================
...
BEGIN
  ...
  -- 3) Fail-closed: escopo desta onda ----------------------------------------
  ...
  -- 4) Tag estratégica interna e subtipo de venda -----------------------------
  ...
  -- 5) Orçamento — leitura dos valores brutos --------------------------------
    ...
    FOR v_idx IN 0..(v_n_adsets - 1) LOOP
      ...
      BEGIN
        IF v_b IS NOT NULL AND v_b > 0 THEN
          v_total_adset_b := v_total_adset_b + v_b;
          v_has_adset_b := true;
        END IF;
      END;
    END LOOP;
  ...
  -- 6) Decisão CBO vs ABO ----------------------------------------------------
  IF v_strategy_tag = 'testing' THEN
    v_budget_mode := 'ABO';
    v_final_cmp_cents := NULL;
    IF v_has_adset_b THEN
      -- Mantém budgets existentes por conjunto.
      FOR v_idx IN 0..(v_n_adsets - 1) LOOP
        v_per_adset := v_per_adset || jsonb_build_object(v_idx::text, NULLIF(v_adsets->v_idx->>'daily_budget_cents', '')::bigint);
      END LOOP;
    ELSIF v_campaign_cents IS NOT NULL AND v_campaign_cents > 0 AND v_n_adsets > 0 THEN
      -- Distribui igualmente, resto no último (determinístico).
      DECLARE
        v_base bigint := v_campaign_cents / v_n_adsets;
        v_rem  bigint := v_campaign_cents - v_base * v_n_adsets;
      BEGIN
        FOR v_idx IN 0..(v_n_adsets - 1) LOOP
  ...
  ELSE
    ...
    FOR v_idx IN 0..GREATEST(v_n_adsets - 1, 0) LOOP
      EXIT WHEN v_n_adsets = 0;
      v_per_adset := v_per_adset || jsonb_build_object(v_idx::text, NULL);
      -- Estimativa informativa apenas se existia valor original.
      DECLARE v_b bigint := NULLIF(v_adsets->v_idx->>'daily_budget_cents', '')::bigint;
      BEGIN
        IF v_b IS NOT NULL AND v_b > 0 THEN
  ...
  -- Atualiza cada adset com orçamento correto e estimativa
  IF v_n_adsets > 0 THEN
  ...
  -- 10) Persiste APENAS action_data. status e lifecycle ficam intocados.
...
COMMENT ON FUNCTION public.ads_patch_proposal_to_v1_1(uuid) IS
  'Onda H.2.1 — Reescreve campaign_proposal para contrato v1.1 de forma idempotente. Não chama IA/Meta, não cria criativos, não altera status nem lifecycle.';
