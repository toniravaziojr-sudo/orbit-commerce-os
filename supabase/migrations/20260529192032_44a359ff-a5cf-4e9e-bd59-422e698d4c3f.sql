...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- AUTO-EMISSÃO BY STATUS: também re-enfileirar quando o pedido atinge
    -- 'ready_to_invoice' vindo de qualquer status anterior diferente.
    -- Isso garante que tenants com fiscal_settings.emitir_apos_status='ready_to_invoice'
    -- tenham o auto-emit disparado no momento certo, mesmo que o rascunho já exista.
  ...
  -- Re-enfileira sempre. Se o rascunho já existir, o consumidor vai apenas avaliar
  -- a condição de auto-emit (sem duplicar rascunho).
  INSERT INTO public.fiscal_draft_queue (tenant_id, order_id)
  VALUES (NEW.tenant_id, NEW.id)
  ON CONFLICT (order_id) DO NOTHING;

  -- Para transições que já tinham rascunho processado (status=done na fila),
  -- precisamos forçar uma nova entrada para reavaliar auto-emit.
  IF v_reached_ready THEN
    INSERT INTO public.fiscal_draft_queue (tenant_id, order_id, status, attempts)
    SELECT NEW.tenant_id, NEW.id, 'pending', 0
    WHERE NOT EXISTS (
      SELECT 1 FROM public.fiscal_draft_queue
