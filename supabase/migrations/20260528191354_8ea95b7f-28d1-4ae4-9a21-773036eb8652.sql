...
BEGIN
  ...
  IF NEW.pedido_status = 'em_aberto' THEN
    ...
    -- Derivar service_code a partir do service_name para Correios quando ausente
...
-- Backfill: corrige rascunhos Correios existentes com service_code nulo
UPDATE shipments s
   SET service_code = CASE UPPER(TRIM(s.service_name))
     WHEN 'PAC' THEN '03298'
     WHEN 'SEDEX' THEN '03220'
     WHEN 'SEDEX 10' THEN '40215'
     WHEN 'SEDEX 12' THEN '40169'
     ...
     ELSE s.service_code
   END
 WHERE s.carrier = 'correios'
   AND (s.service_code IS NULL OR s.service_code = '')
