-- Trava de corrida no espelho Remessa ↔ Pedido de Venda em aberto.
-- 1) Quando o PV sai de em_aberto, cancelar também itens pendentes/processing da fila
--    para impedir que o worker recrie a remessa depois (race condition observada no PV 347 Lesinete).
...
BEGIN
  ...
  -- E cancelar itens da fila ainda pendentes para evitar recriação posterior.
  IF TG_OP = 'UPDATE' AND OLD.pedido_status = 'em_aberto' AND NEW.pedido_status IS DISTINCT FROM 'em_aberto' THEN
    ...
    -- Também limpa por source_pedido_venda_id (cobre PVs sem order_id)
    DELETE FROM shipments
     WHERE source_pedido_venda_id = NEW.id
       AND (tracking_code IS NULL OR tracking_code = '');

    -- Cancela itens pendentes na fila do PV para evitar recriação por retry
