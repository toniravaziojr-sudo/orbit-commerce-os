...
export function featuredProductsToStaticHTML(
  ...
  const productCards = validProducts.map((product, index) => {
    const p = product!;
    ...
    let ratingsHtml = '';
    if (showRatings) {
      const avgRating = p.avg_rating || 4.8;
      const reviewCount = p.review_count || Math.floor(Math.random() * 20) + 15;
    ...
    let buyNowHtml = '';
    if (quickBuyEnabled) {
      buyNowHtml = `<a href="/produto/${escapeHtml(p.slug)}" style="display:block;width:100%;padding:8px;background:var(--theme-button-primary-bg,#1a1a1a);color:var(--theme-button-primary-text,#fff);border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:600;text-align:center;text-decoration:none;">${escapeHtml(buyNowButtonText)}</a>`;
    }
  ...
  return `<section style="max-width:1280px;margin:0 auto;padding:32px 16px;">
    ...
    <style>@media(max-width:768px){.sf-fp-grid{grid-template-columns:repeat(2,1fr) !important;}}</style>
  </section>`;
}
