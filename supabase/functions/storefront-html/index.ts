...
// ============================================
// FOOTER RENDERER (full — from footer_config)
// ============================================
function renderFooter(
  ...
  // Helper functions
  const getString = (key: string, settingsValue?: string | null, fallback?: string | null): string | null => {
    const v = configProps[key];
    if (typeof v === 'string' && v) return v;
    return settingsValue || fallback || null;
  };
  const getBoolean = (key: string, defaultVal = true): boolean => {
    const v = configProps[key];
  ...
  // Footer styles
  const footerBgColor = getString('footerBgColor') || '#1a1a1a';
  const footerTextColor = getString('footerTextColor') || '#e0e0e0';
  const footerTitlesColor = getString('footerTitlesColor') || footerTextColor;
  ...
  if (showSocial && hasSocial) {
    ...
    if (socialYoutube) socialLinks.push(`<a href="${escapeHtml(socialYoutube)}" target="_blank" rel="noopener noreferrer" style="color:${escapeHtml(footerTextColor)};opacity:0.8;" aria-label="YouTube"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19.1c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z"/><polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02" fill="#fff"/></svg></a>`);
...
serve(async (req) => {
  ...
  try {
    ...
    if (route.type === 'home') {
      // HOME — render using Block Compiler (same source of truth as builder)
      const homeContent = publishedContent?.home as BlockNode | null;
      
      if (homeContent) {
        // Extract IDs needed for data fetching from the block tree
        ...
        // Build data maps for compiler context
        const productsMap = new Map(featuredProducts.map((p: any) => [p.id, p]));
        ...
        const categoriesMap = new Map(featuredCategories.map((c: any) => [c.id, c]));
        ...
        // Compile the block tree to HTML (same source of truth as builder!)
      ...
      // LCP preload: find first banner in the tree for responsive preload
