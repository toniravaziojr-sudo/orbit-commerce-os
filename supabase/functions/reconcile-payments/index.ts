...
// Checks pending payments and syncs with Pagar.me API
// ============================================
...
// Fallback to environment secrets for Pagar.me
const ENV_PAGARME_API_KEY = Deno.env.get('PAGARME_API_KEY');

// Minimum age of pending payments to reconcile (in minutes)
...
// Get Pagar.me credentials for a tenant
...
// Fetch order status from Pagar.me API
...
serve(async (req) => {
  ...
  try {
    ...
    const paymentsByTenant = new Map<string, typeof pendingPayments>();
    for (const payment of pendingPayments) {
      if (!paymentsByTenant.has(payment.tenant_id)) {
        paymentsByTenant.set(payment.tenant_id, []);
      }
      paymentsByTenant.get(payment.tenant_id)!.push(payment);
    }
    ...
    for (const [tenantId, tenantPayments] of paymentsByTenant) {
      ...
      if (!apiKey) {
        ...
        stats.skipped_no_credentials += tenantPayments.length;
        continue;
      }
      ...
      for (const payment of tenantPayments) {
        ...
        try {
          if (!payment.provider_transaction_id) {
            console.log(`[reconcile-payments] Payment ${payment.id} has no provider_transaction_id, skipping`);
            stats.unchanged++;
            continue;
          }
          ...
          console.log(`[reconcile-payments] Payment ${payment.id}: Pagar.me status = ${chargeStatus}`);
