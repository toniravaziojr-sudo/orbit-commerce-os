...
async function collectStrategistContext(supabase: any, tenantId: string, configs: AccountConfig[], trigger?: StrategistTrigger) {
  ...
  ((globalThis as any).__strategistTenantSignals ||= new Map()).set(tenantId, tenantStrategicSignals);
...
Execute o pipeline completo de 5 fases. Use strategic_plan para o diagnóstico, depois as tools operacionais.`;
  ...
  try {
    ...
    lines.push("- **USE `catalog_*` APENAS para campanhas de catálogo dinâmico (Advantage+ Catalog / DPA)**. NÃO use `catalog_prospecting`/`catalog_retargeting` para campanhas de vídeo, imagem estática, LAL ou amplo comuns — para essas, use `prospecting` ou `retargeting` simples. Quando for catálogo real, preencha TODO o `catalog_setup` (product_catalog_id + product_set + janela + exclude_recent_buyers_days + creative_mode='dynamic'). Se o catálogo não foi detectado mas a intenção é catálogo, marque `catalog_setup.pending_dependency='catalog_not_connected'` em vez de inventar IDs.");
    ...
    lines.push("- **EXCEÇÃO (teste de produto novo/lançamento):** quando o teste for de produto NOVO ou em LANÇAMENTO (não carro-chefe), você PODE manter a base de clientes no público. Para isso, marque `product_lifecycle='launch'` (ou `'new'`) na ação E deixe `audience_exclusions.customers=false`. Não é necessário `exclusion_override_reason` nesse caso — o sistema reconhece a exceção e libera o plano para aprovação. Para teste de produto CARRO-CHEFE/consolidado, mantenha a exclusão normal.");
